...
class waveDSDataset(HelioNetCDFDataset):
    """
    Template child class of HelioNetCDFDataset showing how to build a downstream dataset.
    Extends the base class with a wave/no-wave label aligned to the Surya index.
    ...
    Additional Args:
        return_surya_stack: If True (default), include the Surya image stack in the returned dict.
            Set to False to return only the wave/no-wave label (useful for label inspection).
        max_number_of_samples: Cap the dataset length at this value. Useful for quick experiments.
        label_transform: Optional callable applied to the column named by ``ds_class_column``
            to produce the ``label`` column. Signature:
            ``(series: pd.Series) -> pd.Series``.  If ``None``, the raw column values are
            used as-is. Define this at the call site (e.g., in ``build_datasets()``) to keep
            normalization logic out of the dataset class.
        ds_wave_index_path: Path to the downstream wave/no-wave CSV index.
        ds_time_column: Column name in the wave index to use as the event timestamp.
        ds_class_column: Column name in the wave index holding the label (e.g. intensity,
            or a class string such as "wave" / "no wave") to pass through ``label_transform``.
    ...
    Raises:
        ValueError: If ``ds_wave_index_path`` or ``ds_class_column`` is not provided, or if
            no overlap exists between the Surya and DS indices within the specified tolerance.
    """
    ...
    def __init__(
        ...
        ds_wave_index_path: str | None = None,
        ds_time_column: str | None = None,
        ds_class_column: str | None = None,
        ...
        **kwargs,
        ...
        if ds_class_column is None:
            raise ValueError("ds_class_column must be provided for waveDSDataset")

        # load_forecast_frames defaults to False here: this task supplies its own labels,
        # so future Surya frames never need to be fetched from disk/S3.
        ...
        if ds_wave_index_path is not None:
            self.ds_index = pd.read_csv(ds_wave_index_path)
        else:
            raise ValueError("ds_wave_index_path must be provided for waveDSDataset")
        ...
        # Apply label transform if provided; otherwise use raw column values.
        if label_transform is not None:
            self.ds_index["label"] = label_transform(self.ds_index[ds_class_column])
        else:
            self.ds_index["label"] = self.ds_index[ds_class_column]
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        Returns:
            Dictionary containing:
                forecast (np.float32): Wave/no-wave label (1.0 / 0.0).
                ds_index (str): ISO-format timestamp from the wave index.
        ...
        sample["forecast"] = self.df_valid_indices.iloc[idx]["label"].astype(np.float32)
