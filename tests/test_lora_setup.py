...
from workshop_infrastructure.models.finetune_models import ClassToken, HelioSpectformer1D
from workshop_infrastructure.utils import HEAD_PREFIX, apply_peft_lora, discover_head_modules

# Small enough to be fast, large enough to exercise both block types.
...
def make_model(pooling="class_token", penultimate_linear_layer=True):
    # nglo counts extra global tokens in the attention windowing. Only the
    # class_token pooling injects one, so nglo must be 1 there and 0 otherwise
    # or the long-short attention reshape fails. The shipped config pairs
    # nglo: 1 with pooling: class_token for this reason.
